...
/*
static int ms5607_configure_sensor()
{
  int ret;

  ret = i2c_read_reg(file_des,0x28, (uint8_t*)&calib_, 26);
  if (ret != 0) { snav_error_print("%s: ms5607 failed to read calibration!!!",DRIVER_NAME); return -1; }

  snav_info_print("%s: ms5607 params %d %d %d    %d %d %d %d %d %d %d %d %d",DRIVER_NAME,
  ...
  if (ret == 0) { snav_info_print("%s: ms5607 sensor config1 ok!",DRIVER_NAME);}
  else          { snav_error_print("%s: ms5607 sensor config1 failed!!!",DRIVER_NAME); return -2; }

  uint8_t config[2] = {0xF5,0b00000000};
  ret = i2c_write_reg(file_des,config[0],&config[1],1);

  if (ret == 0) { snav_info_print("%s: ms5607 sensor config2 ok!",DRIVER_NAME); }
  else          { snav_error_print("%s: ms5607 sensor config2 failed!!!",DRIVER_NAME); return -3; }
...
void ms5607_cmd_reset()
{
	...
	int ret = i2c_write_reg_nddr(file_des,&config[1],1);    //只写入一个cmd 
...
int ret0 = i2c_read_reg(file_des,0xA0, data0, 2);
int ret1 = i2c_read_reg(file_des,0xA2, data1, 2);
int ret2 = i2c_read_reg(file_des,0xA4, data2, 2);
int ret3 = i2c_read_reg(file_des,0xA6, data3, 2);
int ret4 = i2c_read_reg(file_des,0xA8, data4, 2);
int ret5 = i2c_read_reg(file_des,0xAA, data5, 2);
int ret6 = i2c_read_reg(file_des,0xAC, data6, 2);
int ret7 = i2c_read_reg(file_des,0xAE, data7, 2);
...
static uint ms5607_prom(int8_t coef_num)
{
  ...
  //  ms5611ReadCommand(busdev, MS5607_CMD_PROM_RD + coef_num * 2, 2, rxbuf); // send PROM READ command
	...
	int ret = i2c_read_reg(file_des,MS5607_CMD_PROM_RD+coef_num * 2, rxbuf, 2);
...
{
  baro_slave_addr = MS5607_I2C_ADDRESS;
  ...
  if (i2c_slave_config(file_des, baro_slave_addr, baro_bit_rate, baro_trx_timeout_us) != 0)
  {
    ...
    return -2;
	  ...
	  snav_error_print("%s: i2c_slave_config  OK ",DRIVER_NAME);

  }
	usleep(13000);

	uint8_t   data[1] = {255};
	int ret = i2c_read_reg(file_des,0xA0, data, 1);
...
static int ms5607_read_data()
{
  ...
  uint64_t time_read_start  = snav_get_time_1us();
	...
	uint8_t config[2] = {0xee,0x54};
	int retw1 = i2c_write_reg_nddr(file_des,&config[1],1);
	if(retw1!=0)
	snav_info_print("%s  D2  ret = %d",DRIVER_NAME,retw1);

	usleep(10000);
	
	int ret2 = i2c_read_reg(file_des,0x00, data_d2, 3);
	// snav_info_print(" D2 0x%x 0x%x 0x%x   ret=%d",data_d2[0],data_d2[1],data_d2[2],ret2);


	uint8_t configd1[2] = {0xee,0x44};
	int retd1 = i2c_write_reg_nddr(file_des,&configd1[1],1);
	if(retw1!=0)
	snav_info_print("%s D1  ret = %d",DRIVER_NAME,retd1);

	usleep(10000);
	int ret1 =  i2c_read_reg(file_des,0x00, data_d1, 3);
...
//////////////////////////////////////////////////////////////////////////////
	float AccsumX = 0;
		int i;
         ...
         fAccX[FILTERNUM-1] = MS5607_P;
         for( i=0;i<FILTERNUM;i++)
         {
			AccsumX += fAccX[i];
         }
         MS5607_P = AccsumX / FILTERNUM;
