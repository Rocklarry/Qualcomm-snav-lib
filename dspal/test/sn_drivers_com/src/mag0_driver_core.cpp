...
static int mag0_snav_driver_get(int handle, SnavDriverGetEnum type, void * data_ptr, uint32_t data_size)
{
  ...
  else if (type == SNAV_DRIVER_GET_MAG0)
  {
	...
	//snav_info_print("%s: mag_data_out time=%d cntr=%d x=%d y=%d z=%d tem=%d",DRIVER_NAME, mag_data_out.time,mag_data_out.cntr,mag_data_out.field_x,
		//mag_data_out.field_y,mag_data_out.field_z,mag_data_out.temperature);
