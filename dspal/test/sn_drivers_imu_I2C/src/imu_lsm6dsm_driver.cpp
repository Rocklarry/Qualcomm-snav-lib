...
#define DRIVER_NAME "SNAV_IMU_LSM6DSM_DRIVER"
...
static int imu_read_data();

#include "imu1_driver_core.cpp"
#include "lsm6dsm_driver_core.cpp"
...
static int main_driver_thread(void * params)
{
  ...
  char i2c_device[32];
  snprintf(i2c_device, 32, i2c_device_format , imu_port);
  ...
  if (file_des == -1)
  {
    snav_error_print("%s: Failed to open imu port %s. Exiting.",DRIVER_NAME,i2c_device);
    return -1;
  }

  if (lsm6dsm_init())
  {
    snav_error_print("%s: unable to initialize imu",DRIVER_NAME);
    close(file_des);
    file_des = -1;
    return -2;
  }
...
static int snav_driver_set(int handle, SnavDriverSetEnum type, void *  data_ptr, uint32_t data_size)
{
  return imu1_driver_set(handle,type,data_ptr,data_size);
}

static int snav_driver_get(int handle, SnavDriverGetEnum type, void * data_ptr, uint32_t data_size)
{
  return imu1_driver_get(handle,type,data_ptr,data_size);
}
