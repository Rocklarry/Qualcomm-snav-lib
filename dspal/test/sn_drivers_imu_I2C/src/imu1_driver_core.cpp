 ...
 */
static SnavDriverPortTypeEnum  imu_port_type = SNAV_DRIVER_PORT_TYPE_I2C;

static int32_t  imu_port           = 2;
static int32_t  imu_bit_rate       = 400000;
static uint32_t imu_trx_timeout_us = 1000;
static uint32_t imu_slave_addr     = 0;
...
static uint32_t imu_update_time_us = 2000;
...
static int imu1_driver_set(int handle, SnavDriverSetEnum type, void *  data_ptr, uint32_t data_size)
{
  if (type == SNAV_DRIVER_SET_PORT_NUMBER)
  {
    imu_port = *(int32_t*)data_ptr;
    snav_info_print("%s: Updated imu_port param to %d",DRIVER_NAME, imu_port);
    return 0;
  }
  else if (type == SNAV_DRIVER_SET_BIT_RATE)
  {
    imu_bit_rate = *(int32_t*)data_ptr;
    snav_info_print("%s: Updated imu bit rate param to %d",DRIVER_NAME, imu_bit_rate);
    return 0;
  }
  else if (type == SNAV_DRIVER_SET_BLOCKING_IO)
  {
    imu_blocking_read = *(int32_t*)data_ptr;
    snav_info_print("%s: Updated imu to use blocking IO=%d",DRIVER_NAME, imu_blocking_read);
    return 0;
  }
  else if (type == SNAV_DRIVER_SET_UPDATE_INTERVAL_US)
  {
    imu_update_time_us = *(int32_t*)data_ptr;
    snav_info_print("%s: Updated imu update interval param to %d",DRIVER_NAME, imu_update_time_us);
    return 0;
  }
  return -1;
}
...
static int imu1_driver_get(int handle, SnavDriverGetEnum type, void * data_ptr, uint32_t data_size)
{
  ...
  else if (type == SNAV_DRIVER_GET_ACCEL_GYRO1)
  {
    if (imu_blocking_read)
    {
      imu_read_data();
  ...
  else if (type == SNAV_DRIVER_GET_ACCEL_GYRO2)
  {
    if (imu_blocking_read)
    {
      imu_read_data();
