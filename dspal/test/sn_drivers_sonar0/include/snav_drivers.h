...
typedef enum
{
  ...
  SNAV_DRIVER_SET_UPDATE_INTERVAL_US,
...
typedef enum
{
  ...
  SNAV_DRIVER_GET_GNSS0_LOC,
  SNAV_DRIVER_GET_GNSS0_SVINFO,
...
#define MAX_GNSS_CONSTELLATIONS  6
typedef struct
{
  uint64_t time;           //data timestamp in microceconds (DSP)
  uint32_t cntr;           //number of data read by the driver
  uint16_t week;           //gnss week number
  uint32_t tow_sec;        //gnss time of week (seconds)
  uint32_t tow_ns;         //gnss time of week (nanoseconds)
  uint32_t tow_unc_ns;     //gnss time uncertainty (nanoseconds)
  ...
  float    vel_east;       //velocity in East direction
  float    vel_north;      //velocity in North direction
  float    vel_up;         //velocity in Up direction

  float    pos_cov[9];     //position covariance
  float    vel_cov[9];     //velocity covariance
  float    h_pos_accuracy; //horizontal accuracy in meters
  float    h_vel_accuracy; //horisontal accuracy in meters
  float    v_pos_accuracy; //vertical accuracy in meters
  float    v_vel_accuracy; //vertical accuracy in meters
  uint64_t used_in_fix[MAX_GNSS_CONSTELLATIONS]; /* for each constellation the used_in_fix has one bit per sv
                                                            (1 if the svid was used in computing the fix, 0 otherwise) */
} SnavDriversGnssLocData;


#define MAX_GNSS_NUM_SV 64
...
typedef struct
{
  ...
  float    elevation_deg;
  float    azimuth_deg;
} SnavDriversGnssSvInfo;

typedef struct
{
  uint64_t time;         //data timestamp in microceconds
  uint32_t cntr;         //number of data read by the driver
  uint32_t num_sv;       //number of valid satellites
  SnavDriversGnssSvInfo sv_info[MAX_GNSS_NUM_SV];
} SnavDriversGnssSvInfoData;
