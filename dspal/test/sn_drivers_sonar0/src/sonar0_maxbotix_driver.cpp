...
int maxbotix_driver_init()
{
 ...
 // snav_info_print("%s: ====== rrd========  Starting thread, opening device %s at baud rate of %lu",DRIVER_NAME,uart_device,sonar_bit_rate);
  ...
  switch (sonar_bit_rate)
  {
    ...
    default: snav_error_print("%s: unsupported baud rate %lu",DRIVER_NAME,sonar_bit_rate); return -2;
  }

  snav_info_print("%s: Setting baud rate to %d",DRIVER_NAME,sonar_bit_rate);
  if (ioctl(file_des, SERIAL_IOCTL_SET_DATA_RATE, &ioctl_baud_rate) != 0)
  {
    snav_error_print("%s: Failed to set baud rate. Exiting.",DRIVER_NAME);
    close(file_des);
    file_des=-1;
    return -3;
  }
  ...
  maxbotix_receive_callback_config.rx_data_callback_func_ptr = maxbotix_read_callback;

  int result = ioctl(file_des, SERIAL_IOCTL_SET_RECEIVE_DATA_CALLBACK, (void *)&maxbotix_receive_callback_config);
  if (result < 0)
  {
    snav_error_print("%s: failed to set callback",DRIVER_NAME);
    close(file_des);
    file_des=-1;
    return -4;
  }
