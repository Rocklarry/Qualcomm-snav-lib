...
#define DRIVER_NAME "SNAV_BMP280_DRIVER"
...
static int main_driver_thread(void * params)
{
  ...
  char i2c_device[32];
  snprintf(i2c_device, 32, i2c_device_format , baro_port);
  ...
  if (file_des == -1)
  {
    snav_error_print("%s: Failed to open baro i2c port %s. Exiting.",DRIVER_NAME,i2c_device);
    return -1;
  }

  int bmp280_init_ret = bmp280_init();
  if (bmp280_init_ret !=0 )
    return bmp280_init_ret;

  snav_info_print("%s: BMP280 initialization successful!", DRIVER_NAME);
